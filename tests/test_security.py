...
@pytest.mark.parametrize("actor", [Actor("discord", "1482143139828596917"), Actor("discord", ".normal.man."),
    Actor("discord", OWNER_ID, True), Actor("discord", OWNER_ID, False, "webhook"),
    Actor("model", OWNER_ID), Actor("discord", int(OWNER_ID))])
...
async def test_model_cannot_mutate_and_secrets_are_not_discord_editable(kernel):
    ...
    result = await kernel.registry.call("council_inspect", {"resource": "status"}, ToolContext(bot, "room", "turn", True), "call")
...
async def test_vault_encrypts_and_redacts_nested_values(kernel):
    ...
    event = kernel.store.emit("test", {"error": "echo " + secret, "nested": [{"authorization": secret}], "prompt": secret})
    assert secret not in str(event)
    assert "[REDACTED]" in str(event)
    assert "[REDACTED]" in kernel.vault.redact("Bearer abc.def.ghi")


@pytest.mark.parametrize("url", ["http://127.0.0.1", "http://[::1]", "http://169.254.169.254/latest", "http://10.0.0.1", "file:///etc/passwd", "http://localhost", "https://user:password@example.com", "http://example.com:8080", "http://224.0.0.1"])
...
async def test_dns_rebinding_is_checked_at_socket_resolution(monkeypatch):
    ...
    monkeypatch.setattr(loop, "getaddrinfo", AsyncMock(return_value=[(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("127.0.0.1", 443))]))
    with pytest.raises(ControlError):
...
def test_web_session_csrf_and_origin(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        assert client.post("/api/auth/login", json={"password": password}, headers={"Origin": "https://attacker.test"}).status_code == 403
        login = client.post("/api/auth/login", json={"password": password}, headers={"Origin": "http://testserver"})
        ...
        assert client.post("/api/control", json=command, headers={"X-CSRF-Token": csrf, "Origin": "https://attacker.test"}).status_code == 403
...
async def test_discord_owner_gate_ignores_spoofed_admin_and_bot(kernel):
    hortator = kernel.store.get("bots", "hortator")
    for author in (SimpleNamespace(id=123, bot=False, display_name=".normal.man."), SimpleNamespace(id=int(OWNER_ID), bot=True)):
        message = SimpleNamespace(author=author, webhook_id=None, channel=SimpleNamespace(id=456), guild=None, content="!stop all")
...
async def test_discord_command_bypasses_model_and_owner_control_stays_available(kernel, monkeypatch):
    ...
    message = SimpleNamespace(author=SimpleNamespace(id=int(OWNER_ID), bot=False), webhook_id=None,
        channel=SimpleNamespace(id=123), guild=None, content="!stop all", attachments=[])
    await kernel.connector.receive("hortator", message)
...
async def test_application_identity_mismatch_rejected(kernel, owner, monkeypatch):
    ...
    monkeypatch.setattr(kernel.connector, "validate_token", AsyncMock(return_value={"application_id": "222222222222222222", "user_id": "222222222222222222"}))
