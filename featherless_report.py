...
<section><h2>Memory workflow checks independently of final delivery</h2><p>A model can complete every note operation and still fail final-response validation. Counts show steps satisfied in the requested order / attempted cases, across variants. An operation performed out of order may fail its step check even when the final notebook is correct; inspect the native call trace. Unconfirmed final text is not accepted.</p><div class="scroll"><table><thead><tr><th>Model</th><th>Alias</th><th>Read</th><th>Write</th><th>Replace</th><th>Delete</th><th>Read back</th><th>Exact state</th><th>Final answer</th></tr></thead><tbody>{"".join(operation_rows)}</tbody></table></div></section>
