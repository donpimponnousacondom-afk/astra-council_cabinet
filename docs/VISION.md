...
`web_fetch` remains a bounded text extractor, and document imports / outbound artifacts retain their separate limits. There is no built-in Bash or general filesystem tool. The owner deferred shell/filesystem tools for downloading, compressing and reattaching files to the next feature after this PR is merged.
