...
## Readiness and remaining settings
...
The requested follow-up is an accurately labelled, discoverable reasoning configuration location with direct access to the exact JSON and an honest “no explicit override” indication. Preserve arbitrary vendor JSON and the provider/profile separation; do not add an invented inheritance layer or claim to know an undocumented vendor default. The typing branch contains this diagnosis only, with no reasoning UI changes.
...
`CouncilClient.on_disconnect` emits `reconnecting` without a cause; `on_resumed`/`on_ready` emit `online`. Thus `error: null` means no cause was supplied, not a literal error. The data is consistent with brief gateway reconnect/resume behavior, but the stored events do not identify whether Discord or the network initiated it. No recorded pair carried the supervisor's explicit unhealthy-heartbeat message. Notifications report both transitions, with a five-minute per-kind/per-bot throttle, which makes even short recoveries visible. Improved cause recording and less noisy reporting are separate possible hotfix work, not implemented here.
