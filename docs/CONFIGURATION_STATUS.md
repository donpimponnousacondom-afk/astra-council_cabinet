...
**Superseded configuration findings:** later on 2026-09-07, the user confirmed that Hortator's new provider and Kimi K3 work, context configuration is resolved, and Qwen concurrency is set to two requests for four units. The user also confirmed the typing indicator works live. Do not reapply the earlier credential/model/context corrections below: they describe the earlier snapshot, not the current setup. Configuration is considered complete by the user; full council activation/acceptance is still theirs to perform. Use the dashboard for current configuration, [backup procedures](OPERATIONS.md#backup-and-restore) for the latest snapshot pointer, and [VERIFICATION.md](VERIFICATION.md) for dated evidence.
