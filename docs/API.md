...
| `GET /api/background-jobs?bot_id=&plugin=` | All outstanding jobs plus latest 50 settled job states and measurements; optional scope filters |
...
`request.failed` exposes safe `error_origin`, `response_format`, `frame_index` and `field`, alongside actual transport HTTP status, inferred upstream error status and `provider_fault`. Origins distinguish `response_format`, `local_client`, `local_deadline`, `upstream_error`, `upstream_http`, `transport`, `request_configuration` and `cancelled`. Failure/cancellation events also expose `purpose`, `model`, `phase`, `duration_ms` and, for timeouts, `timeout_kind`/`timeout_seconds`. Private `provider_error.details` adds expected/received field types, HTTP timeout phase or bounded local exception locations without locals. Format/local-client/local-deadline errors never increment provider-health counters. Real provider-health failures link back to their triggering bot/turn/request when available. Raw frame text and reasoning remain outside ordinary events. Cancelled compactions emit `compaction.cancelled` at warning level with `previous_context_retained=true`; failed ones remain `compaction.failed` errors.
...
`bots.interval_seconds` accepts 0–86,400 seconds, default 60. Zero disables ordinary scheduler turns, including new unaddressed input and idle evaluations, without disabling gateway intake, authenticated human mention/reply priority, opt-in slash ingress or explicit control actions. `evaluate_when_idle` remains saved but has no scheduling effect at zero. Negative values are invalid; `cooldown_seconds` retains its minimum of one second. The shared control API/Discord JSON editor use the same schema. `!interval <bot> 0` changes only the activation interval; positive values preserve that command's existing combined interval/cooldown update.

Bot responses include derived `application_emojis`: either `status: "unavailable"` with a note, or `status: "ready"`, the verified `application_id`, `total`, `shown`, `truncated`, bounded `items` (`name`, full Discord `markup`) and usage guidance. This is runtime metadata, not a writable bot field. The same bot-scoped catalog reaches ordinary/slash model prompts. No endpoint uploads, deletes or changes ownership of application emojis. See OPERATIONS for refresh, bounded inventory and failure semantics.

Bots expose `memory_char_limit`, a strict integer in **1–128,000**, default **48,000** for older records without that field. Save it using `action: "save", kind: "bots", id: "ada", data: {"memory_char_limit": 12000}`. Zero, negative, fractional and unlimited values are rejected. Each bot/channel permits a temporary 5% aggregate overshoot; while over budget, memory writes must reduce usage. Owner memory controls use the same rules. A configuration save cannot lower the allowed total below retained notes including this headroom; it returns the affected channel and measured usage. Excluding `memory` from `enabled_plugins` disables both the model tool and automatic note injection without deleting notes. See [memory operations](OPERATIONS.md#private-memory-budgets) and [tool results](TOOLS.md#private-memory-allowance-and-repair).
...
`profiles.compaction_max_tokens` is a strict positive integer or null (default). It must be smaller than the profile context window. After stripping inherited JSON caps, a non-null value sends native `max_tokens` only for compaction; null leaves it absent. Old records default to null without changing saved JSON or generation. Compaction planning reserves the larger of this explicit allowance and the retained-text limit.

Compaction request context includes `compaction_output_policy: "provider_default" | "explicit_max_tokens"`, `compaction_max_tokens`, `omitted_output_cap_fields`, `retained_summary_token_limit` and `summary_tokenizer`. `compaction.summary_checked` includes the output policy/cap, `summary_tokens`, retained limit/tokenizer, reported output/reasoning counts, finish reason and `accepted`. Acceptance is for one candidate; all batches and final context fitting must pass before the summary/checkpoint commits. Failed/incomplete candidates remain inspectable under the usual authentication/redaction rules.
...
`GET /api/global-memory/{bot_id}` returns the owner's view of `notes`, `budget` and enablement without requiring a channel context. Authenticated/CSRF-protected `POST` accepts the `global_memory` tool's `operation`, `key` and `value` contract; `{}` is nonmutating usage. Writes/deletes are serialized through owner control and cancel only that bot's current turn. Bot configuration exposes strict integer `global_memory_char_limit` (1–128,000, default 48,000), independent of `memory_char_limit`. Plugin-off notes remain owner-editable. See [GLOBAL_MEMORY.md](GLOBAL_MEMORY.md).

Slash ingress uses Discord's authenticated gateway, not a public HTTP interaction endpoint. Public bot configuration includes `slash_commands` metadata (eligibility, grant/global enablement, owned registration and personal installation URL). Plugin inventory identifies `capability_type: "input"` versus `"tool"`; input capabilities cannot be invoked through model tool APIs. No interaction response token is exposed or persisted. Existing bot/plugin configuration endpoints control grants.
