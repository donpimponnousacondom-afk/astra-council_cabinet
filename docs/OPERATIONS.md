...
- `images/`: private image cache used by the shared multimodal pipeline; originals within the intake limits, disclosed replacements for JPEGs resized from above 64 MP. See [VISION.md](VISION.md).
...
- `jobs/`: isolated execution metadata and retained stdout/stderr logs; `jobs/archive/` keeps automatically archived metadata by original job ID.
...
### Warning and error diagnosis

Folded warnings/errors allow up to 640 characters, prioritizing operation, channel and explanation before secondary metrics (ordinary INFO remains 320). `notice`/`note`, handler, phase, retry, site, exit and output-finish fields are visible when present. Different channels, operations and publication sites are not combined as one repeating incident. Use `f`, `T` or `P` for retained detail; console filtering and folding never change the event ledger.

- **`Local total request deadline exceeded: 120 seconds`** means Hortator cancelled its own request timer, even if the provider was sending tokens. The number is the saved provider setting. `error_origin=local_deadline`, `timeout_kind=total`, `purpose`, `phase`, elapsed duration and request identity explain which attempt stopped. Change **Providers → Edit → Total request timeout (seconds)** for future requests if more time is intended. This setting applies to generation and compaction, independently of retained-summary tokens. No timeout settings are changed by the logging update, and this failure does not increment shared provider health or open its circuit.
- **HTTP timeouts** name connect, read, write or pool. Read means waiting for response data, not an SSE parsing error; pool means waiting for a local HTTP connection slot and does not count as an upstream outage. Real transport failures and upstream errors retain their existing health behavior. Discovery reports its own 20-second HTTP timeout separately from completion settings.
- **Compaction interruption** is `compaction.cancelled` at WARNING; an actual failed attempt remains ERROR. Both retain the previous checkpoint. Request IDs link provider failures to compaction/turn events and private partial evidence. An interrupted summary is never adopted.
- **Discord typing** identifies its separate local deadline or exception and says the bot turn continues. A failed presence update retries on its existing interval; a stopped typing task is distinguished from that recoverable update. Receive/callback errors retain message/channel or handler identity plus redacted traceback.
- **Jobs, deliveries and publishing** retain infrastructure errors, command exit codes, uncertain Discord acceptance, site/revision/job identity and known retry timing. No inspection reruns commands or assumes a failed transfer means the remote received nothing. Restarted uncertain sends are not automatically replayed.
- **Background loops and resource limits** say whether the next tick/check will retry or an owned background task stopped. Loop-lag diagnostics identify active bots only as correlation, not proof of which task blocked the loop. Empty exception messages remain identifiable by type rather than producing a blank error.
...
**Disable the timer without disabling the bot:** set **Bots → Edit bot → Model & rhythm → Activation interval (seconds)** to **0**, or tell Hortator `!interval loki 0` (replace the stable bot ID as needed). The bot remains connected and observes its granted rooms, but ordinary messages, other bots, historical replay and idle evaluation cannot start scheduled model turns. New live human mentions/replies and granted `/prompt` invocations still work within the existing authorization, concurrency, retry and budget gates. Deterministic owner commands and explicit compaction remain available. A failed directed turn is not automatically retried after its priority claim is consumed; send a new mention/reply or slash request to ask again.

The idle toggle is ignored while the timer is off and its saved value is preserved. Setting a positive interval restores the previous scheduling behavior: with idle evaluation off, unhandled messages from other participants still qualify; with it on, an existing conversation may also receive idle turns. Negative intervals are rejected. Zero is specific to the activation timer, not a zero send cooldown or memory budget. `!interval <bot> 0` preserves the current send cooldown; positive `!interval` values keep setting both fields as before. Use `!set bots loki {"interval_seconds":60}` or the dashboard to restore a positive activation interval without changing the send cooldown. Zero applies to Hortator too if explicitly configured: plain owner questions then require a real mention/reply to activate its model, while its deterministic commands stay available.
...
## Application-owned custom emojis

Emojis uploaded to an application's Developer Portal **Emoji** page belong to that application, independently of server emojis. The runtime reads each connected bot's own inventory after verifying its token/application identity, before initial gateway connection, and then every five minutes. Uploading, renaming or deleting an emoji is discovered on the next refresh; reconnecting that bot refreshes immediately. This is read-only discovery, requires no additional plugin grant and does not give bots emoji upload/delete or reaction tools. [Discord application emoji documentation](https://docs.discord.com/developers/resources/emoji#application-owned-emoji).

Every ordinary generation and slash generation receives verified names and full markup under `Runtime facts → application_emojis`. Static emojis use `<:name:id>` and animated ones `<a:name:id>`. The model must copy that markup into ordinary answer text. Bare `:name:` is not automatically expanded by the API/runtime; escaping it or enclosing it in inline/fenced code intentionally displays text. Native mention suppression does not alter emoji markup. JSON Unicode escapes such as `\u003c` decode to `<` and are not evidence of a Markdown escaping bug.

Discovery accepts at most Discord's 2,000 application entries and excludes unavailable emojis. To bound smaller model contexts, guidance shows at most 64 alphabetically sorted entries / 6,000 serialized item characters, with explicit total/shown/truncated fields. The authenticated bot API exposes the same bounded derived inventory; it is not saved bot configuration. When entries are omitted, the model must not invent their IDs. Existing known server-emoji markup remains supported, subject to Discord's server permissions.

The in-memory catalog is tied to the bot's current application ID. Replaced clients cannot install late results; application changes cannot reuse another catalog. A failed or invalid refresh clears that bot's catalog, retries after 60 seconds and reports `discord.application_emojis_failed` with the operation/application/cause while ordinary chat continues. A catalog older than ten minutes is omitted. Successful initial/changed inventories emit `discord.application_emojis_updated` with a count; unchanged refreshes are quiet. Discovery has a separate 20-second local timeout, uses the existing Discord client/rate-limit handling and is cancelled/joined with the runtime. Fresh caches are rebuilt after snapshot restoration/restart; the exact inventory used in a model request remains in its retained request evidence.
...
### Private memory budgets

Configure **Bots → Edit bot → Capabilities → Private memory budget (characters per channel)** in the modern dashboard. `bots.memory_char_limit` is a strict integer from **1 to 128,000**, default **48,000** for new and older records without the field. Zero, negative, fractional and unlimited sentinel values are invalid. This is a per-bot setting applied independently to each channel, not a provider/model setting. The defaults and 5% allowance live in `hortator/memory_budget.py`; validation lives in `hortator/models.py`. No environment variable or restart is needed for a saved bot setting.

Uncheck **Private memory** in the bot's capabilities to remove its tool and automatic note injection. The global plugin switch applies to every bot as well. Stored notes remain intact and owner-inspectable in Context; re-enabling restores access. This does not erase old transcripts, request evidence or summaries that already contain a previously discussed fact. Other tools have their own independent checkboxes. Legacy dashboard functionality is frozen.

The normal budget counts Unicode characters in note **values**, not tokens, keys or serialized JSON. Writing an existing key replaces that value. Each note still permits at most **8,000 characters**. Both model tool writes and owner dashboard/Discord note controls use the same accounting. Lowering a budget cannot silently discard notes: if any channel exceeds the proposed budget plus its 5% headroom, the save names the channel and current usage and asks you to consolidate first.

Small overshoots can use **5% temporary headroom**, rounded down. For example, the unchanged default allows **48,000 → 50,400** stored characters. With that budget, an accepted write at 48,128 is saved with a warning, not reported as a failed tool call. While above the configured budget, subsequent writes must strictly reduce the total; deletes are also allowed. Reductions can span several keys/calls, with warnings until usage is back within budget. Writes exceeding the configured budget plus headroom are rejected without changing notes. Merely reading, starting another turn or waiting does not renew headroom. There is no automatic summarization or extra provider call to fix notes.

Enabled bots see current usage, remaining space, allowance and repair instructions in their memory tool description, `{}`/error usage, read/write/delete results (`budget`) and a refreshed prompt layer each round. Over-budget mutations emit `memory.budget_warning` in console scope **c**, stating the accepted change, usage and required consolidation. Existing notes and the separate retained-summary/context/token budgets are preserved.

### Retained conversation summaries
...
**Model profiles → Edit profile → Context & retained summary → Compaction output cap (max_tokens)** sets optional `compaction_max_tokens`. Blank/null uses the provider default; a positive integer sends that exact native `max_tokens` for **reasoning plus summary**, independently of the retained-text limit and normal replies. For example, 32,768 requests that total output allowance from an endpoint accepting `max_tokens`; it does not guarantee 32,768 visible summary tokens. The value must be smaller than the profile context window, and compaction batches reserve the larger of this allowance and the retained-summary limit, plus the existing margin. Leave enough room for the existing summary and input. Do not use 32,768 on a 32,768-token context.

Default behavior remains unchanged. `max_tokens`, `max_completion_tokens` and `max_output_tokens` in generation JSON or compaction override JSON are still removed before applying the separate explicit field, so previously ignored settings do not become active accidentally. Their saved values, native reasoning settings and normal generation behavior remain unchanged. Clear the dedicated field to return to the provider default. Compaction events and request evidence record `compaction_output_policy` and `compaction_max_tokens`.

Omitting the cap does not remove the endpoint's defaults, model/context constraints, response limits or configured deadlines. Event #30973 on this installation reported 8,192 output tokens and `finish_reason=length` from CheapSeek with no cap sent. This is evidence for that request, not every provider's default. A `length` response still cannot be assumed complete. [DeepSeek's API](https://api-docs.deepseek.com/api/create-chat-completion/) documents combined input/generated context limits and model-specific output defaults; proxy behavior must be checked from actual request/response evidence.
...
| `local_client` | Python adapter/client failure or local HTTP connection-pool wait timeout. Private diagnostics show exception type and available function/line locations or timeout phase, without locals. This does not count as provider downtime. |
| `local_deadline` | Hortator's configured total request deadline expired. Includes duration, timeout setting and request phase; streamed output does not extend this deadline. This does not count as provider downtime. |
...
| `transport` | HTTP connect/read/write timeout, connection/protocol failure or incomplete stream. Partial reasoning is retained; usage may be incomplete. This excludes our total deadline and local pool wait. |
...
**Advanced · compaction parameter overrides** starts with Model parameters and replaces the top-level keys supplied in `compaction_request_json`. For example, a compaction `reasoning: {"effort":"low"}` replaces the entire generation `reasoning` object. The editor previews the resulting reasoning fields. Output-cap fields here remain ignored; use the separate **Compaction output cap (max_tokens)** field. **Retained summary limit** checks completed text locally, excluding reasoning. A request trajectory records the effective request under the existing redaction rules.

**Response token reserve** is space reserved by context planning. It does not set a generation cap. The card's **Generation output cap** shows any explicit `max_tokens`/`max_completion_tokens` in Model parameters; configure the supported field if you want that limit sent for normal replies. The separate **Compaction total-output cap** row displays either the explicit `max_tokens` or provider-default policy. Saved generation JSON is unchanged.
...
Disable pixels for a single bot with **Bots → Capabilities → Receive image inputs** (`allow_images:false`). Other bots may continue using the same model profile with images enabled. The bot keeps attachment metadata and written conversation context; disabling does not purge shared cached files. Image-count/byte budgets remain per profile. See [VISION.md](VISION.md#per-bot-image-inputs).

**Keep recent messages** is the verbatim tail retained *after* compaction, not an intake/history limit. Older uncompressed messages are summarized in bounded batches. A newly assigned bot with no completed checkpoint can therefore have a backlog larger than its model window; that backlog is not sent in one request. Failures/cancellations preserve the old checkpoint. Use an empty experimental channel to avoid importing an existing conversation; never reset checkpoints or erase notes as an implicit optimization.
...
Backups now include `workspaces/`, `jobs/` and `fetched_documents/`, with their SQLite scope/reference/evidence tables. Restore the matching directories and database/key into empty external storage; stopped-runtime snapshots are needed for file/DB consistency. Job recovery marks unfinished commands interrupted, never replayed. Default inactive retention is 30 days for workspaces and seven days for fetched snapshots/job logs; active jobs/running-turn references are protected. Cleanup occurs on new work admission, so time expiry is an eligibility/read-access limit, not a promise of immediate physical erasure. The recent-job count limit now automatically archives the oldest completed records outside running turns into `jobs/archive/`, preserving IDs/history and freeing slots. Archived logs still follow the same age/byte limits; workspace files and trajectory evidence are unchanged. In Plugins → Isolated Bash, **Command time limit (seconds)** accepts up to 600 (default remains 90); shell registry timing follows it, while parent task deadlines still apply. Immutable tool-result evidence follows existing trajectory retention. See the [storage policies](AGENTIC_TOOLS.md#inspection-storage-and-retention) before archiving data.
...
Add `--tools all --tool-suite baseline` for isolated simulated fetch/search/email and memory-alias tests. Use `--tools memory,annotations`, `--tool-guidance simple`, or `--tool-prompt-file /path/to/text` to compare names/instructions without editing the harness. Tool mode groups inference by model, with a default 16-second switch gap: this account also enforces four model switches per minute. All tools are fixtures; the only external effects are the explicitly requested Featherless inference/metadata calls. Per-case JSON notebooks, exact requests and `report.html` stay in the private benchmark directory. Regenerate/aggregate with `uv run python featherless_report.py /path/to/study`. Do not restart the council server for tester-only changes.

## Provider request retries

**Providers → Edit → Limits**, directly below **Failures before circuit opens / Recovery delay**, has **Retries per model request** (`retry_count`, integer 0–10, default 3) and **Retry delay (seconds)** (`retry_delay_seconds`, 0–3600, default 10). Three retries means at most **four attempts**. Zero disables request retries. Older records without the fields receive the same effective defaults; this installation's existing providers are explicitly set to 3 / 10 during the rollout. Other provider settings and credentials are preserved.

A retry repeats only the failed inference request using the same prompt, tools and existing completed tool results. No tool executes from a partial response; no completed tool or Discord message is replayed. Ordinary turns, slash invocations and compaction use this shared behavior. Each logical completion retains its provider-concurrency slot during the cancellable wait; cost checks run before every attempt. Every actual attempt has its own request row, usage/partial diagnostics and linked `retry_group_id`, `attempt`, `max_attempts`, `previous_request_id`. Reported charges from failed attempts remain accounted for; unknown usage remains unknown.

Eligible failures: connection/read/write/protocol interruptions, incomplete streams, malformed provider responses, local request deadlines/pool waits, HTTP 408/409/425/429 and 5xx, and equivalent transient/unspecified upstream error envelopes. Request/authentication/permission errors such as 400/401/403/404/413, local payload/response limits, invalid configuration, disabled providers and cancellation are not retried. `Retry-After` can lengthen the configured delay. The existing total request timeout applies to each attempt; the owning turn/task timeout and 14-minute slash deadline still bound the whole invocation. Stop controls interrupt either I/O or the retry wait. No work resumes automatically after the owning task has ended.

`request.failed` is WARNING when another attempt is available, with `will_retry: true`; `request.retry_scheduled` records the next attempt and delay. Final exhaustion is ERROR. Cancellation during the wait emits `request.retry_cancelled`. Only exhausted/nonretryable logical failures feed the separate provider circuit counter; intermediate attempts cannot open a circuit and abort their own recovery. A successful attempt resets existing provider failure state. Transport diagnostics now retain bounded nested/grouped exception causes and socket errno where available.
