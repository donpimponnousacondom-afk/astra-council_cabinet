...
Small overshoots can use **5% temporary headroom**, rounded down: **48,000 → 50,400** maximum stored characters. An accepted write at 48,128 is saved with a warning, not reported as a failed tool call. While above 48,000, subsequent writes must strictly reduce the total; deletes are also allowed. Reductions can span several keys/calls, with warnings until at most 48,000 remain. A write that would exceed 50,400 is rejected without changing notes. Merely reading, starting another turn or waiting does not renew headroom. There is no automatic summarization or extra provider call to fix notes.
