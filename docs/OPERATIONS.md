...
Preparation tokenization runs off the event loop, checking the full batch first and bisecting when necessary. `compaction.batch_prepared` reports preparation time, message count and tokenization probes in scope **c**. A failure with `finish_reason=length`, zero visible characters and reasoning usage equal to the summary cap means the model spent its allowance without completing a summary. Inspect its saved request using **P** or the private trajectory, then adjust the profile's summary limit/native compaction override as needed. The runtime does not change those settings automatically.
