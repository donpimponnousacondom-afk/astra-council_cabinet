...
Global-memory scope is explicitly **private to each bot, shared across that bot's channels**; it is not a council-wide notebook. Slash ingress must be independently granted and cannot change the director's ordinary intake or confer director-only tools. Stage-specific contracts and validation are recorded when those implementations land.
