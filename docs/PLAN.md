...
Loki's application emoji investigation identified missing metadata discovery rather than a mismatched bot token or escaped outbound markup. Add read-only application-owned emoji discovery through each bot's authenticated client and share the verified bounded catalog with ordinary/slash prompt construction. Preserve native Discord output rather than guessing IDs or converting arbitrary shortcodes. Inventories are ephemeral derived metadata, refresh automatically, and never mix applications. Large catalogs are explicitly bounded for small model contexts; emoji management/reactions are outside this change. See OPERATIONS for usage and diagnostics.

Loki testing exposed the need to suspend scheduled activations independently of connection and command availability. The owner selected zero on the existing activation interval: no routine or idle model turns, while genuine new human mentions/replies, granted slash requests and explicit owner controls remain available. Observation and context retention continue. Positive intervals preserve the existing cadence and idle policy, with no migration or implicit changes to bot records. The timer-only zero sentinel does not extend to memory quotas or delivery cooldowns. The modern editor, shared schema and `!interval` command expose the same behavior; see OPERATIONS for the precise command differences.

The owner requested three independently committed stages from the current stable source: (1) named application snapshots and dashboard restoration, (2) a separate global-memory plugin, and (3) an opt-in slash-command ingress plugin for a companion such as Loki. Existing Hortator behavior, personas, channel grants and normal private notes remain authoritative. No existing bot or global plugin is enabled implicitly. Each stage has its own source commit; deployment follows the final committed build.

Snapshots quiesce all application writers and capture SQLite, its matching key and managed stores outside source Git. Manifests record source/schema identity and checksums. Restore refuses incompatible versions, first captures recovery state, and leaves the runtime paused for inspection. Selected-bot restoration defaults to channel notes only; context checkpoints and that bot's global notebook require separate choices. Newer shared transcripts and external Discord/provider/remote-site effects are not erased by selective recovery. The current implementation uses immutable snapshot directories rather than Git/LFS around database or secrets. See [SNAPSHOTS.md](SNAPSHOTS.md).

Global-memory scope is explicitly **private to each bot, shared across that bot's channels**; it is not a council-wide notebook. Slash ingress must be independently granted and cannot change the director's ordinary intake or confer director-only tools. The separate `global_memory` plugin is now implemented with a per-bot 1–48,000-character quota, 5% temporary headroom, source attribution and automatic injection only when granted. The modern bot editor exposes a Global notes tab even before a bot has a channel context. See [GLOBAL_MEMORY.md](GLOBAL_MEMORY.md). The third stage implements keyless `slash_commands` ingress for council companions: owner-only `/prompt`, user/server installations, public replies by default with explicit `private:true` on owner request (2026-09-13), fresh isolated invocations, shared bot concurrency and a 14-minute platform deadline. It reuses existing generation/tools without reading ambient channel history or widening Hortator intake. See [SLASH_COMMANDS.md](SLASH_COMMANDS.md).
...
The optional tool laboratory compares native calling behavior across eight simulated tool names, including five identical private-memory aliases. Every case starts from an independent JSON notebook and explicit read/write/replace/delete/read-back tasks. Keep the tester's hardcoded runtime-guidance copy and simpler editable alternatives isolated from production prompts/plugins. Record request/schema/prompt variants, actual tool effects, reasoning provenance and full failures; never interpret text-shaped calls as execution or infer a model defect from an upstream rejection. Tool-mode scheduling completes each model before switching, respecting the observed four-switches-per-minute account restriction as well as weighted concurrency. The self-contained HTML report distinguishes setup exclusions, native operation success, complete task success and transport/template diagnostics.
