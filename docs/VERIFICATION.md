...
- **221 focused backend tests passed** across one-shot admission/lifecycle, ordinary runtime, timer-off behavior, addressing, typing, slash commands, panels, reasoning viewer, concurrency, gateway notifications and dispatch. Two existing dependency deprecation warnings remain. The final task-registration adjustment was followed by **23 one-shot tests passing** again. Coverage includes paused/running bots, silence, unchanged saved revisions, cancellation before/after task entry, admission guards, request retries inside one turn, temporary gateway teardown, history readiness and a 30-second readiness deadline with no inference on failure. These are isolated fixtures and controlled transports, not live Discord/provider acceptance.
- **Two modern browser tests passed** on isolated port 18341: Trigger admission/cancellation and existing navigation. Browser checks verify active/paused state, timer-off labeling, busy-button disablement and every action fitting inside its table cell. Visually inspected the resulting screenshot after reserving space for the extra button. TypeScript/Vite build, Ruff check/format and diff checks passed. Legacy source and saved bot/provider configuration were not changed.
- Read-only pre-deployment checks found no active turns or provider requests. No live Trigger, Discord send, paid inference, context reset or schema change was performed for testing. Deployment uses the committed-source shared-Screen `--refresh` workflow; its external `logs/next-feature.json` receipt records the final matching dashboard/server identity. No push is part of this task.

## Reasoning viewer duplicate controls and loading recovery (2026-09-18)

- Investigated live event **#39509** on the existing `feat/reasoning_viewer` branch. The affected Loki answer and provider request were completed/sent, with 426 characters of native reasoning still saved; only the private viewer failed. Serializing a single-request/single-page viewer reproduced four identical `custom_id` values, including disabled buttons. Discord requires distinct IDs across components in one message ([component reference](https://docs.discord.com/developers/components/reference#custom-id)). The earlier test transport did not enforce that constraint.
- Navigation now includes each control's direction in its ID while accepting previously saved page handles. Post-acknowledgement failures attempt one plain private status edit with rejected controls/attachments cleared, preserving saved diagnostics and avoiding new inference. If that notice also fails, a separate warning identifies the bot/interaction and explains how to dismiss/reopen the viewer. Raw Discord errors remain in evidence alongside the failed phase and plain explanation. Owner authorization, footer behavior and grants are unchanged.
- **148 focused tests passed** across reasoning viewer, panels, slash commands, runtime and footers. New coverage validates actual serialized ID uniqueness at every first/middle/last boundary, follows emitted controls through the private dispatcher, reproduces Discord 50035 and connection failures after acknowledgement, and verifies notice-failure logging without a public fallback or replay. Ruff format/check and diff checks passed. No live Discord message/click or paid inference was injected; the owner can dismiss old loading placeholders and click their original REASONING buttons after deployment.
- No active turns/requests at the deployment check. Commit/build/restart uses shared-Screen `--refresh` with its external matching-version receipt; no schema, credential, configuration or live diagnostic edits are required.

## Opt-in private Discord reasoning viewer (2026-09-18)

- Created `feat/reasoning_viewer` from clean, fast-forwarded main `f455963`, after the previous PR was merged. Implemented the eight approved points as the independent keyless `reasoning_viewer` capability, disabled globally and per bot by default. Source buttons bind confirmed answers to saved generation requests; exact-owner clicks read private pages or a complete selected-request text download. No inference, tool execution, transcript injection or footer changes are involved.
- **200 focused backend tests passed** across reasoning viewer, panels, slash commands, ordinary runtime, private diagnostics, footers, snapshots, plugins and concurrency. Cases include disabled defaults, final/earlier request selection, partial/missing/opaque captures, exact owner/application/channel/message/request checks, restart persistence, duplicate opens, revocation during acknowledgement/preparation, cancellation, current credential masking, Unicode pagination, download limits, uncertain acknowledgement recovery and coexistence with streamed/buffered answers, panels and attachments. Two existing dependency deprecation warnings remain. These are isolated fixtures and controlled transports, not live Discord/provider acceptance.
- **Two modern workbench browser tests passed** on isolated port 18341: independent reasoning-viewer opt-in/save/reopen/revocation and existing interactive panels. TypeScript/Vite production build, Ruff and diff checks passed. The legacy frontend and footer implementation were not modified.
- Before schema work, stopped the verified shared-Screen runtime and created offline full backup `/home/codexy/.local/share/hortator-backups/20260918-before-reasoning-viewer`. Read-only timestamps confirm the backup finished before the subsequent runtime start. No live bot grants, memories, provider settings or credentials were changed for testing.
- Deployment follows the committed-source shared-Screen `--refresh` workflow; the external `logs/next-feature.json` receipt records the final dashboard/server identity. No live Discord message or paid inference is injected for acceptance. Enable the plugin globally and for a chosen bot, then test a new answer's private REASONING control with the owner account. No push is part of this task.
...
- Raised the allowed global/per-bot command ceiling to 600 seconds without changing the 90-second default/saved settings. The model schema advertises its effective configured limit; registry shell.run allowance now follows it (up to 630 seconds), and worker export allowance remains 10 seconds. Other tool deadlines and parent task/cancellation rules are unchanged.
- **170 backend tests passed, one optional package-network test skipped**, across real shell execution, archival, agentic API/runtime/delivery, workspaces, console, tool feedback, plugins and snapshots. Coverage includes repeated admission beyond a tiny count budget, original output/status reads, cross-bot/channel/turn refusal, protected running turns, byte quotas, expired archive output, process restart, failed/partially completed archive moves, links/duplicate metadata rejection and full backup restoration. Actual short sandbox commands exercised the 600-second configuration and a captured 610-second host timeout; a controlled registry test verifies the 630-second envelope. This is not a claim that a ten-minute command was run to completion. Two existing dependency deprecation warnings remain.
- **Two modern workbench browser tests passed** on isolated port 18341: save/reopen 600 seconds, reject 601 via the API/native field, show automatic archive guidance, and preserve existing file/job evidence inspection. Production TypeScript/Vite build, Ruff check/format, Prettier and diff checks passed. No legacy source changed.
- Created consistent runtime-maintenance snapshot `20260917T062949444910Z-c3715719877b` before deployment: 989 files / 1,454,588,491 bytes. Receipt: external `$HORTATOR_DATA_DIR/logs/shell-archival-backup.json`. Read-only checks found no active turns/requests and the owner had changed the count to 300; preserve that value, seven-day retention and 90-second command setting. No synthetic shell jobs or provider calls were injected into live bot state for acceptance.
- Deployment follows the committed-source shared-Screen refresh; its external `logs/next-feature.json` receipt records final dashboard/server identity. Automatic archival occurs on the next admission needing a recent-record slot; archived metadata remains part of normal `jobs/` backups.
...
## Interactive Discord panels and document export (2026-09-17)

- Branched clean `909264c` to `feat/interactive_discord_panels`. Added an independent keyless panel tool, persisted action definitions, native Components V2 presentation, owner-only button/dropdown ingress through the shared interaction runner, and single-file document export into existing artifact delivery. No workspace/image grant is implied and no attachment toggle was introduced.
- **134 focused backend tests passed**, including 23 new panel/export cases plus slash, ordinary delivery, runtime and snapshots. Synthetic tests cover exact owner/application/message/channel checks, application changes during acknowledgement, revoked grants, expired/unknown/unconfirmed actions, dropdown choices, restart persistence, duplicate suppression, busy admission, operator cancellation, provider-request retry without replay, ordinary and slash presentation, automatic full-answer attachments, preserved canonical transcript text, pinned owned exports and cross-turn artifact refusal. No live Discord panel was posted or clicked for these tests.
- **16 modern workbench browser tests passed**, including the independent capability/preview, existing draft protection, configuration editing, trajectory and artifact inspection. Inspected the captured gold-accent preview. TypeScript/Vite production build, Ruff check/format and diff checks passed. The legacy frontend was not modified.
- Broad backend regression run: **1,150 passed, one skipped, one pre-existing failure** (two dependency warnings). The remaining baseline failure is `test_five_attempts_and_three_missing_receipts_stop_without_work`: base `909264c` already uses `ACK_ATTEMPTS=30` and `ACK_RETRY_DELAY=0.090`, while its test expects five. This task preserves those existing acknowledgement settings; reconciliation remains separately scoped. After the final application-identity guard, the focused 134-test set passed.
- Created a consistent full pre-deployment snapshot through the runtime maintenance API: `20260917T001544994462Z-dfc57b2e21c2`, 1,012 files / 1,363,849,447 bytes, source `909264c1b055f65b94b4cc55fb64ef584a069582`. The API stopped/joined writers and resumed the existing runtime. Snapshot metadata is outside source Git; the backup receipt is `$HORTATOR_DATA_DIR/logs/interactive-panels-backup.json`.
- Read-only pre-deployment checks found no active provider requests. Loki had document/web/slash/global-memory grants, no workspace or image-generation grant. Hortator had its existing richer grant set. Live Discord rendering/click acceptance remains an owner test; provider/Discord behavior above is controlled transport evidence. The standard clean-branch refresh records final deployed identity in `$HORTATOR_DATA_DIR/logs/next-feature.json`.
...
- **32 targeted tests passed** covering previous tester behavior, all five memory aliases, explicit usage and aggregated validation, replacement/deletion/read-back, native tool IDs and reasoning continuation, merged/separate system-layer layouts, fake-only web/email effects, text-shaped call rejection, partial SSE evidence, retry without tool replay (including HTTP524), HTML/control-unit escaping, estimated/failed-attempt token accounting, matched comparison scopes, standalone controls and disclosed study exclusions. Ruff and diff checks passed. The tester defaults to Hortator's merged system layout and empty-object usage schema wrapper; explicit alternative flags preserve the initial laboratory requests and support controlled comparisons.
...
- Completed the owner-approved 16-model study: **500 tool cases**, **1,422 started inference attempts** and 14 additional requests cancelled before a local slot. The matched baseline is **85/128 complete workflows**, with nine models passing all eight tasks. Mythos also passed 8/8 with all tools exposed. TURBO/JonathanColetti missed strict workflow steps while making valid calls; focused short-guidance repairs passed. These short tests do not establish universal reliability or statistical significance.
- Kimi performed its native tool operations but omitted the final completion boundary. A fresh no-tool `Reply with OK.` control reproduced visible `OK` with missing/null `finish_reason` in both thinking-enabled SSE and JSON; thinking-off completed plain text but failed the native-tool controls. Four other candidates (OBLITERATUS, Gemma26, llmfan Gemma31 and Gryphe Gemma31) failed the tested repair settings, usually returning text-shaped calls. Official Gemma31 passed. All original errors, partial evidence and model outputs remain recorded; no production parser was relaxed.
- Buffered Mythos evidence exposed the supplied `System message must be at the beginning.` HTTP400 behind a generic SSE `bad_request` in the separate-system/thinking-off setup. Merged layers admitted the request but did not make thinking-off execute tools. Low-effort controls worked; high-effort outcomes were mixed and include a real HTTP524 timeout. That exposed a standalone retry omission, now fixed for all 5xx and shared with no-tool tests. A fresh live replay retried after HTTP503 but received capacity errors twice; **524 recovery is verified with a controlled transport, not claimed live**.
- `report.html`, `findings.md`, `study-summary.json`, exact per-case prompts/results, isolated notebooks and private request evidence are complete in that external study directory. Reasoning counts in this sample are labelled cl100k estimates or unavailable, not fabricated upstream counts. New source hashes are captured at module import; `selection.json` discloses the earlier long-lived-process/disk-hash limitation. The report keeps the matched baseline separate from tuning, folds individual records and preserves standalone controls. Final browser checks cover 16 matrix rows/500 case records, filtering, mobile overflow and JavaScript errors; no provider tests remain running.
