...
HTTP capture now precedes extraction for every status. `http_response` retains upstream status/reason/header metadata and a paged original body reference; local snapshot `status: ready` is independent. `read_response(document_id, offset, length)` reads the original response JSON using the same bot/channel/expiry boundary. Unextractable or interrupted responses retain their evidence and a separate local issue. URL validation permits 65,536 characters; console URLs are not shortened. See [HTTP_EVIDENCE.md](HTTP_EVIDENCE.md) for exact capture/retention and credential boundaries.
