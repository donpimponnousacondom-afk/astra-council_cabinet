...
- **Current task branch:** `feat/runtime_version`, created from main after `git pull --ff-only origin main` advanced it to `c42f0c3`. [PR #5](https://github.com/donpimponnousacondom-afk/astra-council_cabinet/pull/5) was squash-merged by the user at 19:52:07 UTC; its tree exactly matched the local refinements tip `90671eb`. The older local `feat/council_refinements` branch was retained; no deletion was requested in this task. No branch was pushed. After a merged PR, the standing workflow is clean tree → fast-forward main → new `feat/`, `hotfix/` or `dev/` branch, with the runtime stopped before checkout. See [OPERATIONS.md](OPERATIONS.md#repeating-the-squash-merge-workflow) for copy/paste commands. Keep new commits linear; the user handles PRs and **Squash and merge**. **NEVER PUSH TO MAIN.** No push is authorized by this handover. Do not create local implementation/merge commits on main, bypass the guard, force-push, or rewrite existing commits. Additional hook work is deferred.
...
- **Latest: `20260907T124253Z-before-refinements`**: stopped-runtime snapshot of the operator-confirmed working configuration, taken with no active turns or pending deliveries before refinement edits. SQLite integrity, all nine file hashes, owner-only permissions and decryption of **13** encrypted records passed. This is the current rollback point; `.latest-requested` points here.
...
The earlier refinements deployment was PID `422205`; that process was stopped cleanly with one Ctrl-C after its active Hortator turn finished, before updating main for this task. Do not reuse that PID. Rediscover the current foreground process before control, and use `!version`/`!ver`, the dashboard banner or authenticated `/api/version` for the loaded commit. The current feature uses a startup snapshot for the server and an embedded build snapshot for the dashboard. Commit before final build/restart; otherwise the uncommitted-build label is intentional. The configured external data and existing verified backup locations are unchanged.
