...
class CouncilClient(discord.Client):
    ...
    async def on_ready(self):
        ...
        if not self.backfill_task or self.backfill_task.done():
            self.backfill_task = asyncio.create_task(self.manager.backfill(self), name=f"backfill:{self.bot_id}")
    ...
    async def on_message(self, message):
        try:
            ...
            self.manager.store.emit("discord.receive_failed", {"error": str(exc)}, bot_id=self.bot_id, level="error")

    async def on_raw_message_delete(self, payload):
        existing = self.manager.store.one("SELECT deleted FROM messages WHERE discord_id=?", (str(payload.message_id),))
        if existing and not existing["deleted"]:
            self.manager.store.execute("UPDATE messages SET deleted=1 WHERE discord_id=?", (str(payload.message_id),))
            self.manager.store.emit("message.deleted", {"message_id": str(payload.message_id), "channel_id": str(payload.channel_id)})

    async def on_raw_message_edit(self, payload):
        existing = self.manager.store.one("SELECT * FROM messages WHERE discord_id=?", (str(payload.message_id),))
        content = payload.data.get("content")
        if existing and content is not None and content != existing["content"]:
            content = self.manager.vault.redact(content)
            self.manager.store.execute("UPDATE messages SET content=? WHERE discord_id=?", (content, str(payload.message_id)))
            self.manager.store.emit("message.edited", {"message_id": str(payload.message_id), "before": existing["content"], "after": content})

    async def on_thread_join(self, thread):
        self.manager.store.emit("discord.thread_joined", {"channel_id": str(thread.id), "parent_id": str(thread.parent_id)}, bot_id=self.bot_id)

    async def on_error(self, event_method, *args, **kwargs):
        self.manager.store.emit("discord.handler_error", {"handler": event_method}, bot_id=self.bot_id, level="error")
...
class DiscordManager:
    ...
    async def validate_token(self, token):
        try:
            async with httpx.AsyncClient(trust_env=False, follow_redirects=False, timeout=15) as client:
                ...
                application = await client.get("https://discord.com/api/v10/oauth2/applications/@me", headers=headers)
                ...
                if application.status_code != 200 or user.status_code != 200 or not user.json().get("bot"):
                    raise ControlError("Discord rejected the bot token. Copy the Bot token, not the OAuth client secret.")
                return {"application_id": application.json()["id"], "user_id": user.json()["id"], "username": user.json()["username"]}
    ...
    def gateway(self, bot_id, status, error=None):
        ...
        self.store.execute("UPDATE bot_runtime SET gateway_status=?,error=coalesce(?,error) WHERE bot_id=?", (status, self.vault.redact(error) if error else None, bot_id))
        if previous["gateway_status"] != status or error and previous["error"] != error:
            self.store.emit("discord." + status, {"error": error}, bot_id=bot_id, level="error" if status == "failed" else "warning" if status == "reconnecting" else "info")
    ...
    async def run_client(self, bot, token):
        ...
        while not self.closed:
            ...
            try:
                ...
                self.gateway(bot["id"], "failed", "Invalid token" if isinstance(exc, discord.LoginFailure) else "Enable Message Content Intent in the Discord Developer Portal")
                return
            ...
            try:
                ...
                for bot in self.store.list("bots"):
                    ...
                    if token and bot["application_id"] and (bot["enabled"] or bot["role"] == "hortator"):
                        ...
                        if bot["id"] not in self.runners:
                            self.runners[bot["id"]] = (fingerprint, asyncio.create_task(self.run_client(bot, token), name=f"discord:{bot['id']}"))
                ...
                if time.time() - self.last_health_check > 15:
                    ...
                    for bot_id, client in list(self.clients.items()):
                        healthy = client.is_ready() and not client.is_closed() and math.isfinite(client.latency) and client.latency < 30
                        self.store.execute("UPDATE bot_runtime SET heartbeat_ms=? WHERE bot_id=?", (client.latency * 1000 if math.isfinite(client.latency) else None, bot_id))
                        self.unhealthy[bot_id] = 0 if healthy else self.unhealthy.get(bot_id, 0) + 1
                        if self.unhealthy[bot_id] >= 3:
                            self.gateway(bot_id, "reconnecting", "Gateway heartbeat is unhealthy; reconnecting")
    ...
    def scope(self, bot, message):
        ...
        if bot["role"] == "hortator":
            ...
            if not message.guild or guild_id == settings["control_guild_id"] and (channel_id == settings["control_channel_id"] or parent_id == settings["control_channel_id"]):
                return f"owner:{bot['id']}"
            return None
        if not message.guild:
            return None
        for room in self.store.list("rooms"):
            if room["id"] in bot["room_ids"] and guild_id == room["guild_id"] and (channel_id == room["channel_id"] or room["include_threads"] and parent_id == room["channel_id"]):
                if message.webhook_id:
                    return None
                known = {self.vault.get(f"bot/{b['id']}/user_id") or b["application_id"] for b in self.store.list("bots")}
                if message.author.bot and str(message.author.id) not in known and not room["allow_external_bots"]:
                    return None
    ...
    async def receive(self, bot_id, message, *, historical=False):
        ...
        known = next((b for b in self.store.list("bots") if str(message.author.id) == (self.vault.get(f"bot/{b['id']}/user_id") or b["application_id"])), None)
        self.store.ingest(discord_id=str(message.id), channel_id=str(message.channel.id), room_id=scope,
            author_id=str(message.author.id), author_name=message.author.display_name, bot_id=known["id"] if known else None,
            ...
            attachments=[{"id": str(a.id), "filename": a.filename, "url": a.url, "size": a.size, "content_type": a.content_type} for a in message.attachments])
    ...
    async def backfill(self, client):
        ...
        if not bot or bot["role"] == "hortator":
            return
        channel_ids = {r["channel_id"] for r in self.store.list("rooms") if r["id"] in bot["room_ids"] and r["channel_id"]}
        channel_ids.update(row["channel_id"] for row in self.store.rows("SELECT channel_id FROM contexts WHERE bot_id=?", (bot["id"],)))
        for channel_id in channel_ids:
            try:
                channel = client.get_channel(int(channel_id)) or await client.fetch_channel(int(channel_id))
                if isinstance(channel, discord.ForumChannel):
                    continue
                last = self.store.one("SELECT discord_id FROM messages WHERE channel_id=? ORDER BY at DESC LIMIT 1", (channel_id,))
                after = discord.Object(id=int(last["discord_id"])) if last and last["discord_id"].isdigit() else None
                count = 0
                async for message in channel.history(limit=5000 if after else 100, after=after, oldest_first=True if after else False):
                ...
                if not after and count:
                    for message in reversed(collected):
                        await self.receive(bot["id"], message, historical=True)
                self.store.emit("discord.history_imported", {"channel_id": channel_id, "observed": count, "initial_tail": not bool(after)}, bot_id=bot["id"])
                if count == 5000:
                    self.store.emit("discord.history_gap", {"channel_id": channel_id, "error": "Backfill reached 5,000 messages; older downtime history may be incomplete"}, bot_id=bot["id"], level="warning")
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                self.store.emit("discord.history_failed", {"channel_id": channel_id, "error": str(exc)}, bot_id=bot["id"], level="warning")
    ...
    async def reply(self, channel, result):
        ...
        if len(content.encode("utf-16-le")) <= 3800:
            await channel.send(content=content if isinstance(result, str) else "```json\n" + content + "\n```", allowed_mentions=discord.AllowedMentions.none())
        else:
            await channel.send(content="Council report attached.", file=discord.File(io.BytesIO(content.encode()), filename="council-report.txt" if isinstance(result, str) else "council-report.json"), allowed_mentions=discord.AllowedMentions.none())

    async def command(self, bot, message):
        actor = Actor("discord", str(message.author.id), message.author.bot, str(message.webhook_id) if message.webhook_id else None)
        actor.require_owner()
        ...
        try:
            if text.startswith("dm "):
                ...
                if not text.startswith("!"):
                    ...
                    self.store.ingest(discord_id=f"private:{message.id}", channel_id=str(channel.id), room_id=f"owner:{bot['id']}",
                        author_id=OWNER_ID, author_name="The Boss", content=text)
            ...
            if name == "help":
                result = "Only The Boss (" + OWNER_ID + ") can use these commands. JSON/text may be attached as a file. Credentials are dashboard-only.\n\n" + "\n".join(f"{cmd} — {desc}" for cmd, desc in COMMANDS)
            elif name == "status":
                status = self.service.status()
                result = {"enabled": status["settings"]["enabled"], "active_requests": status["active_requests"],
                          "bots": [{"id": b["id"], "enabled": b["enabled"], "profile": b["model_profile_id"], **b["runtime"], "readiness": b["readiness"]} for b in status["bots"]],
                          "providers": [{"id": p["id"], "enabled": p["enabled"], **p["health"]} for p in status["providers"]]}
            elif name == "stats":
                ...
                if name == "set":
                    ...
                    kind, entity_id = rest.split(maxsplit=1)
                    data = {}
                result = await self.service.control(actor, {"action": "save" if name == "set" else name, "kind": aliases.get(kind, kind), "id": entity_id, "data": data})
                ...
                data = {"model_profile_id": value.strip()} if name == "use" else {"persona": value} if name == "prompt" else {"interval_seconds": float(value), "cooldown_seconds": float(value)}
                result = await self.service.control(actor, {"action": "save", "kind": "bots", "id": bot_id, "data": data})
            elif name == "clone":
                args = rest.split(maxsplit=1)
                result = await self.service.control(actor, {"action": "clone", "id": args[0], "data": {"name": args[1]} if len(args) > 1 else {}})
            elif name in ("context", "compact"):
                args = rest.split()
                result = self.service.inspect("context", args[0], args[1] if len(args) > 1 else None) if name == "context" else await self.service.control(actor, {"action": "compact", "id": args[0], "data": {"channel_id": args[1]} if len(args) > 1 else {}})
            elif name == "memory":
                bot_id, raw = rest.split(maxsplit=1)
                result = await self.service.control(actor, {"action": "memory", "id": bot_id, "data": self.parse_json(raw)})
                ...
                result = await self.service.control(actor, {"action": "probe" if name == "check" else "reset_circuit", "id": rest.strip()})
            elif name == "thread":
                room_id, title = rest.split(maxsplit=1)
                result = await self.service.control(actor, {"action": "thread", "id": room_id, "data": {"name": title}})
            else:
                raise ControlError("Unknown command. Use !help.")
            self.store.emit("discord.command", {"actor": actor.label, "command": name, "channel_id": str(channel.id)})
            await self.reply(channel, result)
        except Exception as exc:
            error = self.vault.redact(str(exc)) or "Invalid command arguments. Use !help."
            self.store.emit("discord.command_failed", {"actor": actor.label, "error": error}, bot_id=bot["id"], level="warning")
    ...
    async def send(self, bot, channel_id, content, reply_to, paths):
        ...
        try:
            ...
            reference = discord.MessageReference(message_id=int(reply_to), channel_id=int(channel_id), fail_if_not_exists=False) if reply_to else None
            async with asyncio.timeout(60):
                sent = await channel.send(content, files=files, reference=reference, mention_author=False, allowed_mentions=discord.AllowedMentions.none())
            return str(sent.id)
        except discord.HTTPException as exc:
            raise DeliveryError(f"Discord HTTP {exc.status}: {exc.text}", uncertain=exc.status >= 500) from exc
    ...
    async def create_thread(self, room, name):
        ...
        bot = next((b for b in self.store.list("bots") if b["id"] in self.clients and (room["id"] in b["room_ids"] or b["role"] == "hortator")), None)
        if not bot:
            raise ControlError("Connect an application with access to this room first")
        client = self.clients[bot["id"]]
        channel = client.get_channel(int(room["channel_id"])) or await client.fetch_channel(int(room["channel_id"]))
        if str(channel.guild.id) != room["guild_id"]:
            raise ControlError("Room channel does not belong to the configured guild")
        if isinstance(channel, discord.ForumChannel):
            created = await channel.create_thread(name=name, content="Council discussion opened by The Boss.", allowed_mentions=discord.AllowedMentions.none())
    ...
    async def notifications(self):
        ...
        for event in events:
            ...
            reportable = event["kind"] in ("provider.failure", "provider.recovered", "discord.failed", "discord.reconnecting", "discord.online", "turn.failed", "compaction.completed", "delivery.unknown", "delivery.failed", "activation.budget_blocked")
            ...
            if not client or not client.is_ready() or not settings["control_channel_id"]:
                continue
            try:
                channel = client.get_channel(int(settings["control_channel_id"])) or await client.fetch_channel(int(settings["control_channel_id"]))
                ...
                info = {k: v for k, v in event["data"].items() if k in ("error", "provider_id", "consecutive_failures", "retry_in_seconds", "after_tokens", "before_tokens")}
                await self.reply(channel, f"[{event['kind']}] {event['bot_id'] or 'provider'} · event #{event['seq']}\n" + preview(json.dumps(info, ensure_ascii=False), 1300))
                self.last_notification[key] = time.time()
            except Exception as exc:
                self.store.emit("notification.failed", {"source_event": event["seq"], "error": str(exc)}, level="warning")
