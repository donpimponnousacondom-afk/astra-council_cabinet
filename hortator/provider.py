...
def strip_reasoning(content: str) -> str:
    ...
    content = re.sub(r"<(think|thinking|analysis|reasoning)\b[^>]*>.*?(?:</\1\s*>|$)", "", content, flags=re.S | re.I)
    return content.strip()


def safe_payload(value):
    if isinstance(value, dict):
        return {k: ("[reasoning omitted]" if k in {"reasoning_content", "reasoning_details"} else safe_payload(v)) for k, v in value.items()}
...
def normalized_usage(usage, profile):
    ...
    return dict(input_tokens=inp, output_tokens=out, reasoning_tokens=reasoning,
                cached_tokens=cached, cost=cost, cost_source=cost_source)
...
class ProviderPool:
    ...
    async def slot(self, provider):
        ...
        async with self.changed:
            ...
            if health["circuit_until"] > time.time() or pid in self.probing:
                raise ProviderError("Provider circuit is open; waiting for recovery probe", provider_fault=False)
    ...
    def key(self, provider, bot_id):
        return self.vault.get(f"bot/{bot_id}/provider_key") or self.vault.get(f"provider/{provider['id']}/api_key")
    ...
    def success(self, provider):
        ...
        self.store.execute("UPDATE provider_health SET consecutive_failures=0,circuit_until=0,last_error=NULL,last_success=? WHERE provider_id=?", (time.time(), provider["id"]))
        if old["consecutive_failures"]:
            self.store.emit("provider.recovered", {"provider_id": provider["id"], "previous_failures": old["consecutive_failures"]})
    ...
    def failure(self, provider, exc):
        ...
        delay = max(exc.retry_after, provider["circuit_seconds"] if count >= provider["failure_threshold"] else 0)
        error = self.vault.redact(str(exc))[:2000]
        self.store.execute("UPDATE provider_health SET consecutive_failures=?,circuit_until=?,last_error=? WHERE provider_id=?",
                           (count, time.time() + delay if delay else 0, error, provider["id"]))
        self.store.emit("provider.failure", {"provider_id": provider["id"], "consecutive_failures": count,
                        "retry_in_seconds": delay, "error": error}, level="error")

    async def complete(self, *, bot, profile, messages, tools, turn_id, context, purpose="generation", output_limit=None):
        ...
        async with self.slot(provider):
            ...
            meta = {**context, "queue_ms": (clock - queued) * 1000, "profile_revision": profile["revision"],
                    "provider_revision": provider["revision"], "endpoint": provider["base_url"] + "/chat/completions"}
            self.store.execute("""INSERT INTO requests(id,turn_id,bot_id,provider_id,profile_id,model,purpose,
              started_at,status,body,context) VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
              (request_id, turn_id, bot["id"], provider["id"], profile["id"], profile["model"], purpose,
               started, "running", dumps(self.vault.redact(safe_payload(body))), dumps(self.vault.redact(meta))))
            self.store.emit("request.started", {"purpose": purpose, "profile_id": profile["id"], "model": profile["model"],
                            "estimated_tokens": context.get("estimated_tokens")}, bot_id=bot["id"], turn_id=turn_id, request_id=request_id)
            ...
            try:
                ...
                async with asyncio.timeout(provider["timeout_seconds"]):
                    async with self.client.stream("POST", provider["base_url"] + "/chat/completions", headers=headers,
                                                  json=body, timeout=provider["timeout_seconds"]) as response:
                        status_code = response.status_code
                        if response.status_code >= 300:
                            raw = (await response.aread())[:12000].decode(errors="replace")
                            try:
                                retry = float(response.headers.get("retry-after", "0"))
                            except ValueError:
                                retry = 0
                            raise ProviderError(f"HTTP {response.status_code}: {self.vault.redact(raw)[:2000]}",
                                                http_status=response.status_code, retry_after=min(max(retry, 0), 3600),
                                                provider_fault=response.status_code in (401, 403, 429) or response.status_code >= 500)
                        if "text/event-stream" in response.headers.get("content-type", ""):
                            async def packets():
                                ...
                                async for line in response.aiter_lines():
                                    size += len(line)
                                    if size > 8_000_000:
                                        raise ProviderError("Provider stream exceeded the 8 MB response limit", provider_fault=False)
                                    if line == "":
                                        if lines:
                                            yield "\n".join(lines)
                                            lines = []
                            ...
                            async for payload in packets():
                                ...
                                for choice in packet.get("choices", []):
                                    ...
                                    if token or reasoning or details or any(c.get("function", {}).get("arguments") for c in delta.get("tool_calls", [])):
                                        if ttft is None:
                                            ttft = (time.perf_counter() - clock) * 1000
                                            self.store.emit("request.first_token", {"ttft_ms": ttft}, bot_id=bot["id"], turn_id=turn_id, request_id=request_id)
                            ...
                            async for chunk in response.aiter_bytes():
                                ...
                                if total_bytes > 8_000_000:
                                    raise ProviderError("Provider response exceeded the 8 MB limit", provider_fault=False)
                                chunks.append(chunk)
                ...
                if len(result.tool_calls) > 100 or any(not c.get("id") or c.get("type") != "function" for c in result.tool_calls):
                ...
                self.store.execute("""UPDATE requests SET ended_at=:ended_at,status='completed',ttft_ms=:ttft,
                  ...
                  {**metrics, "ended_at": time.time(), "ttft": ttft, "visible": visible, "duration": duration,
                   "usage": dumps(self.vault.redact(result.usage)), "response": dumps(self.vault.redact({"content": result.content,
                   "tool_calls": result.tool_calls, "finish_reason": result.finish_reason})), "http_status": status_code, "id": request_id})
                self.store.emit("request.completed", {**metrics, "duration_ms": duration, "ttft_ms": ttft,
                                "finish_reason": result.finish_reason}, bot_id=bot["id"], turn_id=turn_id, request_id=request_id)
                return result
            except BaseException as error:
                cancelled = isinstance(error, asyncio.CancelledError)
                if not isinstance(error, (Exception, asyncio.CancelledError)):
                    raise
                exc = error if isinstance(error, ProviderError) else ProviderError(
                    "Request cancelled; provider usage may be incomplete" if cancelled else
                    f"{type(error).__name__}: {str(error) or 'Request timed out'}")
                message = self.vault.redact(str(exc))[:3000]
                metrics = normalized_usage(result.usage, profile)
                self.store.execute("""UPDATE requests SET ended_at=:ended,status=:status,error=:error,
                    duration_ms=:duration,ttft_ms=:ttft,http_status=:http_status,usage=:usage,
                    ...
                    {**metrics, "ended": time.time(), "status": "cancelled" if cancelled else "failed", "error": message,
                     "duration": (time.perf_counter()-clock)*1000, "ttft": ttft, "http_status": status_code,
                     "usage": dumps(self.vault.redact(result.usage)), "id": request_id})
                self.store.emit("request.cancelled" if cancelled else "request.failed", {"error": message,
                                "provider_id": provider["id"], "profile_id": profile["id"], "http_status": status_code},
                                bot_id=bot["id"], turn_id=turn_id, request_id=request_id, level="warning" if cancelled else "error")
                if not cancelled:
    ...
    async def probe(self, provider):
        ...
        try:
            response = await self.client.get(provider["base_url"] + "/models", headers=self.headers(provider, ""), timeout=20)
            if response.status_code >= 300:
                raise ProviderError(f"Model discovery returned HTTP {response.status_code}", http_status=response.status_code)
            raw = response.json()
            models = [{"id": item.get("id"), "context_length": item.get("context_length"), "pricing": item.get("pricing")}
                      for item in raw.get("data", [])[:3000] if isinstance(item, dict)]
            # Discovery validates transport/auth only. It must not clear completion failure counters.
            result = {"provider_id": provider["id"], "latency_ms": (time.perf_counter() - start) * 1000,
                      "models": models, "note": "Discovery succeeded. This does not verify chat completions or tool support."}
